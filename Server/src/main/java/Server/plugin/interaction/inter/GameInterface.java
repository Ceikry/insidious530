		...
		switch (component.getId()) {
			...
			case 746:
				switch (button){
					...
					case 43: // Quest tab
						OsrsTab.openInterface(player);
						break;

					case 113: // XP tracker
						switch (opcode) {
							case 155: // Toggle XP Display
								if (player.getAttributes().getOrDefault("xptracker:open", Boolean.FALSE) == Boolean.TRUE) {
									player.getXpTracker().hide();
								} else {
									player.getXpTracker().open();
								}
								break;
							case 196: // Reset
								player.getXpTracker().resetXp();
								break;
							case 124: // setup
								player.getXpTracker().setup();
								break;
						}
						break;
				}
