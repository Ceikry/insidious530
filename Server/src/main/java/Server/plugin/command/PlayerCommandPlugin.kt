...
import plugin.interaction.inter.custom.gamemodes.GameModes
import plugin.interaction.inter.custom.teleports.teleports.TeleportsInterface
...
class PlayerCommandPlugin : CommandPlugin() {
    ...
    override fun parse(player: Player?, name: String?, arguments: Array<String?>?): Boolean {
        when (name) {
            ...
            "arroba" -> {
                GameModes.open(player);
            }
            "arroba2" -> {
                TeleportsInterface.open(player);
            }
            "bankresetpin" -> {
                if (arguments!!.size < 2) {
