...
public abstract class Quest implements Plugin<Object> {
	...
	/**
	 * Returns if the player has not started the quest
	 * @param player The Player
	 * @return true if has started false if not
	 */
	public boolean notStarted(Player player) {
		return player.getQuestRepository().getStage(this) <= 0;
	}

	/**
	 * Returns if the player has started the quest
	 * @param player The Player
	 * @return true if has started false if not
	 */
	public boolean hasStarted(Player player) {
		return player.getQuestRepository().getStage(this) > 0 && player.getQuestRepository().getStage(this) < 99;
	}
