...
public final class ActivityData implements SavingModule {
	...
	/**
	 * The total we donated to well of goodwill
	 */
	private int totalDonatedToWell;
	...
	public boolean isChosenGamemode() {
		return chosenGamemode;
	}

	public void setChosenGamemode(boolean chosenGamemode) {
		this.chosenGamemode = chosenGamemode;
	}

	/**
	 * Did player choose a game mode?
	 */
	 private boolean chosenGamemode;
	...
	public void save(ByteBuffer buffer) {
		...
		SavedData.save(buffer, chosenGamemode, 23);
		SavedData.save(buffer, totalDonatedToWell, 24);
		buffer.put((byte) 0);
	...
	public void parse(JSONObject data){
		...
		chosenGamemode = (boolean) data.get("chosenGamemode");
		totalDonatedToWell = Integer.parseInt( data.get("totalDonatedToWell").toString());
	}
	...
	public void parse(ByteBuffer buffer) {
		...
		while ((opcode = buffer.get()) != 0) {
			...
			case 22:
				...
				break;
			case 23:
				chosenGamemode = SavedData.getBoolean(buffer);
				break;
			case 24:
				totalDonatedToWell = buffer.getInt();
			    break;
			}
		}
	}
	...
	public int getTotalDonatedToWell() {
		return totalDonatedToWell;
	}

	public void setTotalDonatedToWell(int totalDonatedToWell) {
		this.totalDonatedToWell = totalDonatedToWell;
	}
}