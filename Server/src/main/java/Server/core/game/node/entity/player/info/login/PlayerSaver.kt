...
class PlayerSaver (val player: Player){
    ...
    fun saveGameModeData(root: JSONObject){
        if (player.ironmanManager.mode == null)
            return;
        
            root.put("gameMode", player.gamemodeManager.mode.ordinal.toString())
    }
    ...
    fun saveActivityData(root: JSONObject){
        ...
        activityData.put("chosenGamemode", player.savedData.activityData.isChosenGamemode)
        activityData.put("totalDonatedToWell",player.savedData.activityData.totalDonatedToWell.toString())
